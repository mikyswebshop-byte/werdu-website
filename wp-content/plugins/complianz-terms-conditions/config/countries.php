<?php
/**
 * Country, region, and language data for the Terms & Conditions wizard.
 *
 * Populates three properties on the `cmplz_tc_config` instance: `$eu_countries`,
 * `$regions`, and `$countries`. These are used throughout the wizard to build
 * country/region drop-downs, determine which privacy law applies to a given
 * jurisdiction, and resolve ISO 639-1 language codes to human-readable labels.
 * This file is included directly by `cmplz_tc_config` during initialisation.
 *
 ...
 *
 * Includes all 27 EU member states plus Iceland (IS), Liechtenstein (LI), and
 * Norway (NO) from the European Economic Area. Referenced by `$this->regions`
 * below to populate the `eu` region's `countries` list, and used wherever the
 * plugin needs to identify whether a jurisdiction falls under the GDPR.
 *
 * @since  1.0.0
 * @access public
 * @var    string[] $eu_countries Flat array of two-letter country codes.
 */
...
/**
 * Privacy-law region definitions used by the wizard jurisdiction selector.
 *
 * Each key is a short region slug (e.g. `'eu'`, `'us'`). Each value is an
 * associative array with the following keys:
 *
 * - `label`     {string}   Translated display name shown in the UI.
 * - `countries` {string[]} ISO 3166-1 alpha-2 codes that belong to this region.
 * - `law`       {string}   Translated name of the applicable privacy regulation.
 * - `type`      {string}   Consent model: `'optin'`, `'optout'`, or `'optinstats'`.
 *
 * @since  1.0.0
 * @access public
 * @var    array[] $regions Associative array of region slug → region definition.
 */
$this->regions = array(
	'us' => array(
 ...
 *
 * Maps every supported country code to its translated display name. Used to
 * populate the country drop-down throughout the wizard (e.g. company country,
 * target market selection). Some entries use composite codes such as `'GB-EW'`
 * (England and Wales), `'GB-NIR'` (Northern Ireland), and `'GB-SCT'` (Scotland)
 * to allow granular UK jurisdiction selection.
 *
 * @since  1.0.0
 * @access public
 * @var    string[] $countries Associative array of ISO country code → translated country name.
 */
...
/**
 * ISO 639-1 language codes for the wizard language selector.
 *
 * Maps two-letter language codes to their translated display names. Intentionally
 * limited to major European languages to keep the number of translatable strings
 * manageable and to reflect the languages most commonly needed by T&C documents
 * generated by this plugin.
 *
 * @since  1.0.0
 * @access public
 * @var    string[] $language_codes Associative array of ISO 639-1 language code → translated language name.
 */
